...
class _DetailPageState extends State<DetailPage> {
  ...
  void _buyProperty(BuildContext context) async {
    ...
    try {
      ...
      await _payment.makePayment(
          context, advanceAmount.toStringAsFixed(2), "USD");
    } catch (e) {
  ...
  void _navigateoChatScreen(
    ...
    if (receiverId != null) {
      ...
      // If ownerId or gmail is null, show an error
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.homeData['name'] ?? 'Pet Details'),
      ),
      body: Stack(
        children: [
                        ...
                        child: MyButton(
                          onPressed: () {
                            _payment.makePayment(
                                context, widget.homeData['price'], "usd");
                          },
                          text: 'Buy this Pet',
                        ),
                      ),
